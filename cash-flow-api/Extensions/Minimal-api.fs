...
let mapGet (pattern: string) (app: WebApplication) func =
    app.MapGet(pattern, Func<HttpContext, Guid, IResult> func) |> ignore
...
let mapPut (pattern: string) (app: WebApplication) func =
    app.MapPut(pattern, Func<HttpContext, IResult> func) |> ignore

    app

let mapDelete (pattern: string) (app: WebApplication) func =
    app.MapDelete(pattern, Func<HttpContext, IResult> func) |> ignore
